...
Deployment-ready Node.js Vercel Function for sending Smart Planner support requests through Zoho SMTP. It is not connected to the public support navigation yet.
...
1. Create a Vercel project with **Root Directory** set to `support-api`.
2. Add every value from `.env.example` in Vercel Settings → Environment Variables.
...
7. Deploy and map the project to `support-api.feathly.com`.
...
Attachments are intentionally deferred. Vercel Functions have a 4.5 MB request-body limit, so production attachments should upload directly to private object storage with short-lived signed URLs and malware scanning rather than pass through this function.
