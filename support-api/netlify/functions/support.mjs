...
async function verifyTurnstile(token, ip) {
  const body = new URLSearchParams({ secret: process.env.TURNSTILE_SECRET_KEY, response: String(token || ''), remoteip: ip });
  const response = await fetch('https://challenges.cloudflare.com/turnstile/v0/siteverify', { method: 'POST', body, signal: AbortSignal.timeout(6000) });
...
export default async function handler(request) {
    ...
    if (missing.length) {
      console.error('Support API configuration is incomplete', { missing });
      return Response.json({ ok: false, code: 'SERVICE_NOT_CONFIGURED' }, { status: 503, headers });
    }

    const ip = clientIp(request);
    const redis = Redis.fromEnv();
    const rateLimit = new Ratelimit({ redis, limiter: Ratelimit.slidingWindow(5, '10 m'), prefix: 'feathly:support' });
    const limited = await rateLimit.limit(ip);
    if (!limited.success) return Response.json({ ok: false, code: 'RATE_LIMITED' }, { status: 429, headers });
    ...
    const subject = subjectFor(ticket, submission);
    const transport = mailTransport();
    const from = { name: process.env.SUPPORT_FROM_NAME || 'Feathly Support', address: process.env.SUPPORT_FROM_EMAIL };
    try {
      await transport.sendMail({
        ...
        subject,
        text: internalText(ticket, submission),
        html: internalHtml(ticket, submission),
        disableFileAccess: true,
        disableUrlAccess: true
      });
      const ack = acknowledgement(ticket, submission.locale);
      try {
        await transport.sendMail({ from, to: submission.email, replyTo: process.env.SUPPORT_TO_EMAIL, ...ack, disableFileAccess: true, disableUrlAccess: true });
      } catch {
        console.error('Support acknowledgement failed', { ticket });
      }
      return Response.json({ ok: true, ticket }, { status: 200, headers });
    } catch {
      console.error('Support delivery failed', { ticket });
